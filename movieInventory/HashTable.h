...
public:
	...
	//Maps the specified key to the specified value in the hasTable
	...
	//Removies the key(and its corresponding value) from the hashTable
...
template<class KeyType, class ValueType>
inline void HashTable<KeyType, ValueType>::put(const KeyType &key, const ValueType &value)
{
...
template<class KeyType, class ValueType>
bool HashTable<KeyType, ValueType>::containsKey(const KeyType &key)
{
	int index = getHash(key);
...
template<class KeyType, class ValueType>
int HashTable<KeyType, ValueType>::getSize() const
{
	return size;
}

template<class KeyType, class ValueType>
int HashTable<KeyType, ValueType>::getHash(const KeyType &key) const
{
	int hash = 0;
	int length = key.length();

	//Summing up the ascii value of chars to get the index
